...
static const char *const g_julialang_keywords[] = {
...
static std::string JuliaIdentity(const std::string &name) {
  for (size_t i = 0;
       i < sizeof(g_julialang_keywords) / sizeof(g_julialang_keywords[0]); i++) {
    if (name == g_julialang_keywords[i]) { return MakeCamel(name + "_", false); }
  }
...
class JuliaGenerator : public BaseGenerator {
 public:
  JuliaGenerator(const Parser &parser, const std::string &path,
              const std::string &file_name, const std::string &julia_namespace)
 ...
 private:
  ...
  void BuildFieldOfTable(const StructDef &struct_def, const FieldDef &field,
    ...
    if (!IsScalar(field.value.type.base_type) && (!struct_def.fixed)) {
      ...
      code += CastToBaseType(field.value.type, JuliaIdentity(field.name));
    }
  ...
  void BeginFile(const std::string &name_space_name, const bool needs_imports,
    ...
    if (needs_imports) {
      ...
      if (!parser_.opts.julia_import.empty()) {
        code += "\tflatbuffers \"" + parser_.opts.julia_import + "\"\n";
      } else {
...
bool GenerateGo(const Parser &parser, const std::string &path,
  ...
  julia::GoGenerator generator(parser, path, file_name, parser.opts.go_namespace);
