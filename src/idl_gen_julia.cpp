...
static const char *const g_julia_keywords[] = {
...
static std::string JuliaIdentity(const std::string &name) {
  for (size_t i = 0;
       i < sizeof(g_julia_keywords) / sizeof(g_julia_keywords[0]); i++) {
    if (name == g_julia_keywords[i]) { return MakeCamel(name + "_", false); }
  }
...
class JuliaGenerator : public BaseGenerator {
 public:
  JuliaGenerator(
    ...
    // std::istringstream iss(julia_namespace);
    // std::string component;
    // while (std::getline(iss, component, '.')) {
    //   julia_namespace_.components.push_back(component);
    // }
  }
  ...
  bool generate() {
    ...
    BeginFile(LastNamespacePart(julia_namespace_), true, is_enum, &code);
    code += one_file_code;
    code += "\n\nend #module\n\n";
 ...
 private:
    ...
    void GenPropertyNamesAsStruct(const StructDef &struct_def, std::string *code_ptr){
      ...
      for (auto it = struct_def.fields.vec.begin();
         it != struct_def.fields.vec.end(); ++it) {
          auto &field = **it;
          if (field.deprecated) continue;
          code += "struct " + JuliaIdentity(field.name) + " <: AbstractProperty end\n";
      }
      code += "end\n\n";
    ...
    void GenGetPropertyByNameStruct(const StructDef &struct_def, std::string *code_ptr){
      ...
      for (auto it = struct_def.fields.vec.begin();
         it != struct_def.fields.vec.end(); ++it) {
          ...
          code += "function Base.getindex(x::" + struct_def.name + ", ::Type{" +struct_def.name + "Properties." +  JuliaIdentity(field.name) + "})\n";
  ...
  std::string GenTypeBasic(const Type &type) {
    ...
    static const char *ctypename[] = {
      ...
      #define FLATBUFFERS_TD(ENUM, IDLTYPE, CTYPE, JTYPE,  GTYPE, NTYPE, PTYPE, RTYPE, KTYPE, STYPE, JLTYPE) \
        #JLTYPE,
  ...
  bool SaveType(const Definition &def, const std::string &classcode,
    ...
    Namespace &ns = julia_namespace_.components.empty() ? *def.defined_namespace
                                                     : julia_namespace_;
    std::string code = "";
